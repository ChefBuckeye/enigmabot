...
while True:
    ...
    """
    for submission in subreddit.submissions.unread(limit=25):
        ...
        if submission.id not in cache and isMatch:
            print("Match found! Comment ID: " + submission.id)
            secretMessage = encrypt(submission.body) #Encrypts comment before reply
            if str(submission.author) != "theenigmabot": #Prevents replying to self
                if str(submission.author) != "UtdStatBot":
                    try: #Prevent crashing the script
                        submission.reply("I see this is a " + wordLib[random.randrange(0, len(wordLib) - 1)] + " topic, may I suggest a better comment?\nHere I encrypted your message for you so you can edit and replace:\n" + secretMessage + "\nIf you need to decrypt this you can click here. This is an automated message.")
                        print("Replied to " + str(submission.author) + " at " + submission.id)
                        cache.append(submission.id)
                        sleep(30) #Just to prevent overly fast spam
                    except Exception:
                        print("Error! Failed to post to " + submission.id + ": " + submission.body)
                        sleep(60)
                #sleep(900)
            else:
...
"""
Pretty decent code, could use some work, especially with comment duplication
it needs more work to work properly and not reply when its not supposed to.
"""